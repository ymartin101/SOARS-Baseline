...
namespace {
    ...
    /// Function to calculate the angles inside a triangle in n-dimensions
    __host__ __device__ void get_tri_angles(Vec3 P1, Vec3 P2, Vec3 P3, rsFloat* tri_angles)
    {
        Vec3 a = P3-P1;
        Vec3 b = P2-P1;
        rsFloat theta1, theta2, theta3;
        rsFloat d = DotProduct(a,b);
        if (d/(a.Length()*b.Length()) < -1) // Safeguard against slight floating-point values < -1 or > 1
            theta1 = M_PI;
        else if (d/(a.Length()*b.Length()) > 1)
            theta1 = 0;
        else
            theta1 = acos(d/(a.Length()*b.Length()));
        ...
        if (d/(a.Length()*b.Length()) < -1) // Safeguard against slight floating-point values < -1 or > 1
            ...
            theta2 = acos(d/(a.Length()*b.Length()));
        ...
        if (d/(a.Length()*b.Length()) < -1) // Safeguard against slight floating-point values < -1 or > 1
            ...
            theta3 = acos(d/(a.Length()*b.Length()));

        // Return the angles in order of points
        tri_angles[0] = theta1;
        tri_angles[1] = theta2;
        tri_angles[2] = theta3;
    ...
    {
        // NOTE: Need to add code such that the Earth's shape is accounted for, i.e. no direct transmission if there's no line-of-sight from Tx to Rx
        ...
        if (R > std::numeric_limits<rsFloat>::epsilon())
        {
            ...
            // Step 3: Doppler shift is zero (no antenna movement in SOARS)
            results.doppler = 0;  // No relative motion

            // Receiver duals do not receive any direct transmissions; however, real receivers can receive direct transmissions from a transmitter dual
...
__global__ void SolveRE(rsFloat length, REResults* results, rsFloat temp, int targsize, int numPoints, int pulses, Vec3* trpos_arr,
                        Vec3* repos_arr, Vec3* tapos_start_arr, Vec3* tapos_end_arr,
                        ...
                        rsFloat trans_dualfactor, rsFloat recv_dualfactor, rsFloat recv_temp)
{
    ...
    for (int i = tid; i < (targsize*numPoints*pulses); i += stride){ // Will only iterate again if (targsize*numPoints*pulses) > (MaxBlocks*MaxThreads); very unlikely
        ...
        // printf("%e,\n", results[i].power*10000);

        // Step 3: Calculate phase shift; see "Phase Delay Equation" in doc/equations/equations.tex
        results[i].phase = -fmod(results[i].delay*2*M_PI*carrier, 2*M_PI);

        // Step 4: Calculate doppler shift

            // Target's next position
            Vec3 tapos_end = tapos_end_arr[i];

            // If target is not moving
            if ((tapos.x == tapos_end.x) && (tapos.y == tapos_end.y) && (tapos.z == tapos_end.z))
                results[i].doppler = 0; // No relative motion

            else {

                // Monostatic
                if ((trpos_arr[time_idx].x == repos_arr[time_idx].x) && (trpos_arr[time_idx].y == repos_arr[time_idx].y) && (trpos_arr[time_idx].z == repos_arr[time_idx].z)) {
                    
                    // Get delta angle
                    rsFloat Delta_angles[3];
                    get_tri_angles(trpos_arr[time_idx], tapos, tapos_end, Delta_angles); // To get delta
                    rsFloat Delta = Delta_angles[1];  // Delta angle [rad]; if |Delta| > pi/2 rad, Fd will be negative (i.e. target moving away from radar)

                    // Get radial velocity
                    rsFloat V_targ = (tapos_end - tapos).Length()/length;  // Projected target velocity vector (on the bistatic plane)
                    rsFloat V_radial = V_targ*cos(Delta);	// Project the velocity component onto the bistatic bisector, i.e. get radial velocity
                    
                    // Get Doppler shift
                    results[i].doppler = carrier*(((1 + V_radial/c)/(1 - V_radial/c)) - 1);  // Calculates Fd using Doppler effect equation; Fd = Fr - Fc
                }

                // Bistatic
                else {

                    rsFloat tri_angles[3];
                    get_tri_angles(trpos_arr[time_idx], tapos, repos_arr[time_idx], tri_angles);  // Angles from Tx, to Targ0, to Rx
                    rsFloat Beta = tri_angles[1]; // Beta angle in bistatic triangle [rad]

                    // Get tapos_end projected onto bistatic plane (i.e. next target coordinates)
                    Vec3 normal = CrossProduct(tapos - trpos_arr[time_idx], tapos - repos_arr[time_idx]);  // Returns a, b, c for plane
                    normal = normal/normal.Length(); // Unit vector of plane normal
                    Vec3 tapos_end_proj = tapos_end - normal*DotProduct((tapos_end - tapos), normal); // Project onto bistatic plane

                    // Get delta using bistatic bisector and projected tapos_end
                    Vec3 TxT0 = trpos_arr[time_idx] - tapos;	// Shift Tx to tapos as origin
                    Vec3 RxT0 = repos_arr[time_idx] - tapos;	// Shift Rx to tapos as origin
                    TxT0 = TxT0/TxT0.Length(); // Normalise to unit vector
                    RxT0 = RxT0/RxT0.Length(); // Normalise to unit vector
                    Vec3 Bisector = (TxT0 + RxT0) + tapos;  // Add normalised vectors, then shift result back by tapos
                    rsFloat Delta_angles[3];
                    ...
                    // rsFloat Lambda = c/carrier; // Wavelength [m]; needed if using FIRST Doppler equation below
                    rsFloat V_proj = (tapos_end_proj - tapos).Length()/length;  // Projected target velocity vector (on the bistatic plane)
                    rsFloat V_radial = V_proj*cos(Delta)*cos(Beta/2);	// Project the velocity component onto the bistatic bisector, i.e. get radial velocity
                    // results[i].doppler = 2*V_radial/Lambda; // Calculates Fd using the geometry
                    results[i].doppler = carrier*(((1 + V_radial/c)/(1 - V_radial/c)) - 1);  // Calculates Fd using Doppler effect equation; Fd = Fr - Fc
                    // printf("Doppler: %e, %d, %d\n", results[i].doppler, i, time_idx);
                }
            }
...
void rs::SimulatePair(Transmitter *trans, Receiver *recv, World *world, int MaxThreads, int MaxBlocks)
{
    ...
    for (int k = 0; k < pulses; k++){
        ...
        for (int n = 0; n < numPoints; n++){
            ...
            for (int j = 0; j < targsize; j++){
                ...
                rcs_arr[jnk_index] = targ_arr[j]->GetRCS(transvec, recvvec, Wl);
                ...
                // printf("Gt: %.10lf, Gr: %.10lf\n", gt_arr[jnk_index], gr_arr[jnk_index]);
            }
        }
    }
    ...
    if ((targsize*numPoints*pulses) <= MaxThreads){ // If there are fewer targets than the number of threads in one block (or an equal number of targets)
        SolveRE<<<1, (targsize*numPoints*pulses)>>>(sample_time, d_results_arr, temp, targsize, numPoints, pulses, d_trpos_arr,
                                  d_repos_arr, d_tapos_start_arr, d_tapos_end_arr, carrier, c,
        ...
        SolveRE<<<MaxBlocks, MaxThreads>>>(sample_time, d_results_arr, temp, targsize, numPoints, pulses, d_trpos_arr,
                                  d_repos_arr, d_tapos_start_arr, d_tapos_end_arr, carrier, c,
                                  d_rcs_arr, d_gt_arr, d_gr_arr, noprop, trans_dualfactor, recv_dualfactor, recv_temp);
    }
        ...
        SolveRE<<<(((targsize*numPoints*pulses) + (MaxThreads - 1))/MaxThreads), MaxThreads>>>(sample_time, d_results_arr, temp, targsize, numPoints, pulses,
                                  d_trpos_arr, d_repos_arr, d_tapos_start_arr, d_tapos_end_arr, carrier, c,
                                  d_rcs_arr, d_gt_arr, d_gr_arr, noprop, trans_dualfactor, recv_dualfactor, recv_temp);
    }
