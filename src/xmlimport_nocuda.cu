...
void convtime(int year, int mon, int day, int hr, int min, double sec, int timezone, double dut1, int dat,
...
// Based on code written by David Vallado
void polarm(double xp, double yp, double ttt, eOpt opt, double pm[3][3])
{
...
// Based on code written by David Vallado
void teme_ecef(double rteme[3], double vteme[3], double ateme[3], edirection direct,
...
/// Process SGP4 propagation for a target
void ProcessSGP4(pugi::xml_node &handXML, Path *path, double& current_time, double timestep, elsetrec satrec)
{
    try {
        // Initialise local variables
        double r[3], v[3], acc[3], r1[3], v1[3], acc1[3];
        double sec, jd, jdFrac = 0.0;
        int year, mon, day, hr, minute = 0;
        ...
        double time_min = current_time/60; // Convert to minutes
        SGP4Funcs::sgp4(satrec, time_min, r, v);
        ...
        int timezone = 0;
        int dat  = 32;
        double jdut1, ttt, ut1, tut1, jdut1Frac, utc, tai, tt, jdtt, jdttFrac, tcg, tdb, ttdb, jdtdb, jdtdbFrac, tcb;

        // Convert TEME to ECEF
        astTime::days2mdhms(year, satrec.epochdays, mon, day, hr, minute, sec);
        ...
        // Assign coordinates
        Coord coord;
        coord.t = current_time;
        coord.pos = Vec3(x, y, z);
        path->AddCoord(coord);
        current_time = current_time + timestep; // Update current time
    }
    catch (XmlImportException &e) {
        rsDebug::printf(rsDebug::RS_VERBOSE, "WARNING: Something went wrong while propagating target orbit.\n");
    }
}
...
namespace {
    ...
    void ProcessSystemPath(pugi::xml_node &mpXML, Platform *platform)
    {
        ...
        try {
            string rottype = GetAttributeString(mpXML, "interpolation", "");
            if (rottype == "linear")
    ...
    void ProcessDocument(pugi::xml_node &root, World *world, bool included, int MaxThreads, int MaxBlocks)
    {
        ...
        double endtime = GetChildRsFloat(parameters, "endtime");  // Simulation endtime
        ...
        for (pugi::xml_node targ = root.child("target"); targ; targ = targ.next_sibling("target")) {
            ...
            // Get the interpolation type
            try {
                string rottype = GetAttributeString(tmp, "interpolation", "");
                if (rottype == "linear")
                    path->SetInterp(Path::RS_INTERP_LINEAR);
                else if (rottype == "cubic")
                    path->SetInterp(Path::RS_INTERP_CUBIC);
            ...
            double startmfe, stopmfe, deltamin = 0.0;
            elsetrec satrec;
            SGP4Funcs::twoline2rv(longstr1, longstr2, 'm', 'e', 'i', wgs84, startmfe, stopmfe, deltamin, satrec);
            double current_time = 0;
            while (current_time < endtime){
                ProcessSGP4(tleXML, path, current_time, timestep, satrec);
            }
            current_time = endtime;
            ProcessSGP4(tleXML, path, current_time, timestep, satrec); // Final Coord at endtime
        ...
        gettimeofday(&t6, 0);
        double time2 = (1000000.0*(t6.tv_sec-t5.tv_sec) + t6.tv_usec-t5.tv_usec)/1000.0;
        printf("Time for SGP4: %.15lf milliseconds \n", time2);
    }
