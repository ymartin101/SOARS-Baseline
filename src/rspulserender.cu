...
namespace {
    ...
    hid_t OpenHDF5File(std::string recv_name)
    {
        hid_t hdf5_file = 0;
        if (rs::rsParameters::export_binary()) {
          ...
          b_oss << recv_name << ".h5";
          const std::string name = b_oss.str();

          // Open the binary file for writing
          hdf5_file = rshdf5::CreateFile(name);
        }
        return hdf5_file;
    }
    ...
    /// Export to the HDF5 file format
    void ExportResponseSoarsBin(const std::vector<rs::Response*>& responses, const rs::Receiver* recv, const std::string recv_name)
    {
        ...
        hid_t out_bin = OpenHDF5File(recv_name);
...
void rs::ExportReceiverXML(const std::vector<rs::Response*> &responses, const std::string filename)
{
    ...
    pugi::xml_node root = doc.append_child("receiver");

    // Dump each response in turn
    std::vector<rs::Response*>::const_iterator re;
    for (re = responses.begin(); re != responses.end(); re++)
        (*re)->RenderXML(root);

    // Write the output to the specified file
    doc.save_file((filename+".soarsxml").c_str());
...
void rs::ExportReceiverBinary(const std::vector<rs::Response *> &responses, Receiver* recv, const std::string recv_name)
{
    ExportResponseSoarsBin(responses, recv, recv_name);
}
