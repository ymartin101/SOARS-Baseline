...
//struct timeval t1, t2, t5, t6; // For timing calculations
...
// Based on code written by David Vallado
__global__ void SGP4_kernel(rsFloat* x_arr, rsFloat* y_arr, rsFloat* z_arr, rsFloat* tle_skip_arr, elsetrec* satrec_arr, int targsize, int numTimes, rsFloat* time_arr){
    ...
    for (int i = tid; i < (targsize*numTimes); i += stride){ // Will only iterate again if (targsize*numTimes) > (MaxBlocks*MaxThreads); very unlikely
        ...
        rsFloat time_skip = tle_skip_arr[ti];
        rsFloat time_min = time_arr[time_idx]/60 + time_skip; // Get time from array (based on thread index), convert to minutes; add "skip time" from user input (minutes)
        ...
        // printf("[%.10f, %.10f, %.10f, %.10f],\n", x_arr[i], y_arr[i], z_arr[i], time_min);
    }
}
...
namespace {
    ...
    void ProcessDocument(pugi::xml_node &root, World *world, bool included, int MaxThreads, int MaxBlocks)
    {
        ...
        vector<rsFloat> tle_skip_vec(targsize);
        ...
        for (pugi::xml_node targ = root.child("target"); targ; targ = targ.next_sibling("target")) {
            ...
            if (tleXML) {
                ...
                rsFloat TLE_SKIP = GetChildRsFloat(tleXML, "timeskip");
                ...
                satrec_vec[t] = satrec;
                path_vec[t] = path;
                tle_skip_vec[t] = TLE_SKIP;
            }
            else {
                throw XmlImportException("Target " + name + " does not specify a TLE set.");
            }
        ...
        rsFloat *d_y_arr;
        rsFloat *d_z_arr;
        rsFloat *d_tle_skip_arr; // Device array of tle_skip_vec
        ...
        cudaMalloc((void **)&d_tle_skip_arr, sizeof(rsFloat)*targsize);
        cudaMalloc((void **)&d_satrec_arr, sizeof(elsetrec)*targsize);  // Allocate GPU memory
        cudaCheckErrors("Malloc fail");

        // Copy arrays to GPU
        cudaMemcpy(d_tle_skip_arr, tle_skip_vec.data(), sizeof(rsFloat)*targsize, cudaMemcpyHostToDevice); // Copy from host to GPU memory
        ...
        if ((targsize*numTimes) <= MaxThreads){ // If there are fewer targets than the number of threads in one block (or an equal number of targets)
            SGP4_kernel<<<1, (targsize*numTimes)>>>(d_x_arr, d_y_arr, d_z_arr, d_tle_skip_arr, d_satrec_arr, targsize, numTimes, d_time_arr);
        }
            ...
            SGP4_kernel<<<MaxBlocks, MaxThreads>>>(d_x_arr, d_y_arr, d_z_arr, d_tle_skip_arr, d_satrec_arr, targsize, numTimes, d_time_arr);
        }
        else{ // If number of targets requires more than 1 block, but not all of them
            SGP4_kernel<<<(((targsize*numTimes) + (MaxThreads - 1))/MaxThreads), MaxThreads>>>(d_x_arr, d_y_arr, d_z_arr, d_tle_skip_arr, d_satrec_arr, targsize, numTimes, d_time_arr);
        }
        ...
        //gettimeofday(&t6, 0);
        //rsFloat time = (1000000.0*(t6.tv_sec-t5.tv_sec) + t6.tv_usec-t5.tv_usec)/1000.0;
        //printf("Time to SGP4: %.15lf milliseconds \n", time);
    }
...
void xml::LoadXMLFile(string filename, World *world, int MaxThreads, int MaxBlocks)
{
    ...
    // gettimeofday(&t2, 0);
    // rsFloat time = (1000000.0*(t2.tv_sec-t1.tv_sec) + t2.tv_usec-t1.tv_usec)/1000.0;
    // printf("Time to read: %.15lf milliseconds \n", time);
